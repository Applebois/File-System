...
void edit(int currentuserlevel,string username);
void displaylevel(int currentuserlevel);
bool verifyblacklist(string);

int level1(string username,string userdescription,string group)
{
int currentuserlevel=1;
char command;
string filename,directories,makefile,string_fot,console,dump;
filename = "\home\\"+username+"\\";
cout<<"Your menu"<<endl;
cout<<"You enjoying Read & Write privilege ."<<endl;
menu:
cout<<"(C)reate, (R)ead, (W)rite, (L)ist , (S)wapAccount ";
...
case 'W':
case 'w':
    edit(currentuserlevel,username);
    goto menu;
